...
_trial.type_ : 
_pas_ : Indicates how clearly the subject felt to have seen the stimulus, (4 values: from 1 being not seen stimuli to 4 seeing in clearly).

_trial_ : Trial number for each participant
_target.contrast_ : 
_cue_ : 
_task_ :
_target.type_ : Whether the target shown was an even or odd number
_rt.subj_ : Reaction time for subject's response (seconds)
_rt.obj_ : 
_obj.resp_ : Whether the subject indicated that the target was even (*e*) or odd (*o*)
_subject_ : Unique identifier for each participant
_correct_ : Logical argument indicating the subject indicated the correct answer (1 = correct, 0 = incorrect)
...
```{r}
new <- list.files(path = "experiment_2", pattern = "*.csv", full.names = F)
...
model_1 <- glm(correct ~ target.contrast, data = test, family = binomial)
summary(model_1)

test <- test %>%
  mutate(inv = inv.logit(model_1$fitted.values))

summary(model_1)

ggplot(test, aes(x = target.contrast, y = model_1$fitted.values)) +
  geom_point() +
  facet_wrap(~subject)
...
```{r}
model_2 <- glmer(correct ~ target.contrast + (1 + target.contrast | target.contrast), data = df_f, family = "binomial")
